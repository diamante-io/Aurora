...
type diamnetCoreRunnerMode int

const (
	diamnetCoreRunnerModeOnline diamnetCoreRunnerMode = iota
	diamnetCoreRunnerModeOffline
)

// diamnetCoreRunner uses a named pipe ( https://en.wikipedia.org/wiki/Named_pipe ) to stream ledgers directly
// from Diamnet Core
type pipe struct {
	// diamnetCoreRunner will be reading ledgers emitted by Diamnet Core from the pipe.
	// After the Diamnet Core process exits, diamnetCoreRunner should eventually close the reader.
	Reader io.ReadCloser
	// diamnetCoreRunner is responsible for closing the named pipe file after the Diamnet Core process exits.
	// However, only the Diamnet Core process will be writing to the pipe. diamnetCoreRunner should not
...
func newDiamnetCoreRunner(config CaptiveCoreConfig, mode diamnetCoreRunnerMode) (*diamnetCoreRunner, error) {
	var fullStoragePath string
	if runtime.GOOS == "windows" || mode == diamnetCoreRunnerModeOffline {
		...
		// because it's possible it's running multiple Diamnet-Cores on a single
		// machine.
		...
		//    https://github.com/diamnet/go/issues/3437
		// but be sure to re-use rather than replace it:
		//    https://github.com/diamnet/go/issues/3631
...
func writeConf(captiveCoreToml *CaptiveCoreToml, mode diamnetCoreRunnerMode, location string) (string, error) {
...
func generateConfig(captiveCoreToml *CaptiveCoreToml, mode diamnetCoreRunnerMode) ([]byte, error) {
	if mode == diamnetCoreRunnerModeOffline {
		var err error
...
func (r *diamnetCoreRunner) getConfFileName() string {
	joinedPath := filepath.Join(r.storagePath, "diamnet-core.conf")
...
func (r *diamnetCoreRunner) getLogLineWriter() io.Writer {
	rd, wr := io.Pipe()
...
func (r *diamnetCoreRunner) createCmd(params ...string) *exec.Cmd {
...
func (r *diamnetCoreRunner) context() context.Context {
	return r.ctx
}
...
func (r *diamnetCoreRunner) handleExit() {
	defer r.wg.Done()
...
func (r *diamnetCoreRunner) closeLogLineWriters(cmd *exec.Cmd) {
...
func (r *diamnetCoreRunner) getMetaPipe() <-chan metaResult {
...
func (r *diamnetCoreRunner) getProcessExitError() (bool, error) {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *diamnetCoreRunner) close() error {
	...
	if started {
		// wait for the diamnet core process to terminate
		r.wg.Wait()
	...
	if runtime.GOOS == "windows" ||
		...
		r.mode == diamnetCoreRunnerModeOffline {
