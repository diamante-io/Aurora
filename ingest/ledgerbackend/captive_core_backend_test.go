...
func (m *diamnetCoreRunnerMock) context() context.Context {
	a := m.Called()
...
func (m *diamnetCoreRunnerMock) getMetaPipe() <-chan metaResult {
	a := m.Called()
	return a.Get(0).(<-chan metaResult)
}

func (m *diamnetCoreRunnerMock) getProcessExitError() (bool, error) {
	a := m.Called()
...
func TestCaptivePrepareRange(t *testing.T) {
	...
	captiveBackend := CaptiveDiamnetCore{
		archive: mockArchive,
		diamnetCoreRunnerFactory: func(_ diamnetCoreRunnerMode) (diamnetCoreRunnerInterface, error) {
...
func TestCaptivePrepareRangeCrash(t *testing.T) {
	...
	err := captiveBackend.PrepareRange(ctx, BoundedRange(100, 200))
	assert.EqualError(t, err, "Error fast-forwarding to 100: diamnet core exited unexpectedly: exit code -1")
	mockRunner.AssertExpectations(t)
	mockArchive.AssertExpectations(t)
}
...
func TestCaptivePrepareRangeCloseNotFullyTerminated(t *testing.T) {
	...
	err = captiveBackend.PrepareRange(ctx, BoundedRange(100, 200))
	assert.EqualError(t, err, "error starting prepare range: the previous Diamnet-Core instance is still running")
...
func TestCaptivePrepareRange_ErrCatchup(t *testing.T) {
	...
	assert.EqualError(t, err, "error starting prepare range: opening subprocess: error running diamnet-core: transient error")
...
func TestCaptiveDiamnetCore_PrepareRangeAfterClose(t *testing.T) {
	ctx := context.Background()
	executablePath := "/etc/diamnet-core"
	networkPassphrase := network.PublicNetworkPassphrase
	...
	assert.NoError(t, captiveDiamnetCore.Close())

	assert.EqualError(
		t,
		captiveDiamnetCore.PrepareRange(ctx, BoundedRange(65, 66)),
		"error starting prepare range: opening subprocess: error getting latest checkpoint sequence: "+
			"error getting root HAS: Get \"http://localhost/.well-known/diamnet-history.json\": context canceled",
	)
	...
	captiveDiamnetCore.archive = mockArchive
	assert.EqualError(
		t,
		captiveDiamnetCore.PrepareRange(ctx, BoundedRange(65, 66)),
		"error starting prepare range: opening subprocess: error running diamnet-core: context canceled",
	)
	mockArchive.AssertExpectations(t)
}
...
func TestCaptiveAfterClose(t *testing.T) {
	...
	assert.EqualError(t, err, "diamnet-core must be opened to return latest available sequence")
...
func TestCaptiveGetLedgerTerminatedUnexpectedly(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{
			"diamnet core exited unexpectedly without error",
			context.Background(),
			[]metaResult{{LedgerCloseMeta: &ledger64}},
			true,
			nil,
			"diamnet core exited unexpectedly",
		},
		{
			"diamnet core exited unexpectedly with an error",
			context.Background(),
			[]metaResult{{LedgerCloseMeta: &ledger64}},
			true,
			fmt.Errorf("signal kill"),
			"diamnet core exited unexpectedly: signal kill",
		},
		{
			"diamnet core exited unexpectedly without error and closed channel",
			context.Background(),
			[]metaResult{{LedgerCloseMeta: &ledger64}},
			true,
			nil,
		...
		{
			"diamnet core exited unexpectedly with an error and closed channel",
			context.Background(),
			[]metaResult{{LedgerCloseMeta: &ledger64}},
			true,
			fmt.Errorf("signal kill"),
