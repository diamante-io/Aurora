...
// Info calls the `info` command on the connected hcnet core and returns the
// provided response
...
func (c *Client) SetCursor(ctx context.Context, id string, cursor int32) error {
	...
	if body != SetCursorDone {
		return errors.Errorf("failed to set cursor on hcnet-core: %s", body)
	}
...
// WaitForNetworkSync continually polls the connected hcnet-core until it
...
// simpleGet returns a new GET request to the connected hcnet-core using the
