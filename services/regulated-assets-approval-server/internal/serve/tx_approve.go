...
func (h txApproveHandler) handleActionRequiredResponseIfNeeded(ctx context.Context, diamnetAddress string, paymentOp *txnbuild.Payment) (*txApprovalResponse, error) {
	...
	const q = `
		WITH new_row AS (
			...
			VALUES ($1, $2)
			ON CONFLICT(diamnet_address) DO NOTHING
			RETURNING *
		)
	...
	err = h.db.QueryRowContext(ctx, q, diamnetAddress, intendedCallbackID).Scan(&callbackID, &approvedAt, &rejectedAt, &pendingAt)
	if err != nil {
