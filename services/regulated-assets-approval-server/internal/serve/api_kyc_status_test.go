...
func TestAPI_postKYCStatus(t *testing.T) {
	...
	q := `
		INSERT INTO accounts_kyc_status (diamnet_address, callback_id)
		VALUES ($1, $2)
	`
	...
	q = `
		...
		WHERE diamnet_address = $1 AND callback_id = $2
	`
...
func TestAPI_getKYCStatus(t *testing.T) {
	...
	m := chi.NewMux()
	m.Get("/kyc-status/{diamnet_address_or_callback_id}", handler.ServeHTTP)

	// step 1: insert data into database
	const q = `
		INSERT INTO accounts_kyc_status (diamnet_address, callback_id, email_address, created_at, kyc_submitted_at, rejected_at, pending_at, approved_at)
		VALUES
			('rejected-diamnet-address', 'rejected-callback-id', 'xrejected@test.com', $1::timestamptz, $2::timestamptz, $2::timestamptz, NULL, NULL),
			('pending-diamnet-address', 'pending-callback-id', 'ypending@test.com', $1::timestamptz, $3::timestamptz, NULL, $3::timestamptz, NULL),
			('approved-diamnet-address', 'approved-callback-id', 'approved@test.com', $1::timestamptz, $4::timestamptz, NULL, NULL, $4::timestamptz)
	`
	...
	r := httptest.NewRequest("GET", "/kyc-status/rejected-diamnet-address", nil)
	r = r.WithContext(ctx)
	w := httptest.NewRecorder()
	m.ServeHTTP(w, r)
	resp := w.Result()
	...
	wantBody := fmt.Sprintf(`{
		"diamnet_address": "rejected-diamnet-address",
	...
	r = httptest.NewRequest("GET", "/kyc-status/pending-diamnet-address", nil)
	r = r.WithContext(ctx)
	w = httptest.NewRecorder()
	m.ServeHTTP(w, r)
	resp = w.Result()
	...
	require.NoError(t, err)
	wantBody = fmt.Sprintf(`{
		"diamnet_address": "pending-diamnet-address",
	...
	r = httptest.NewRequest("GET", "/kyc-status/approved-diamnet-address", nil)
	r = r.WithContext(ctx)
	w = httptest.NewRecorder()
	m.ServeHTTP(w, r)
	resp = w.Result()
	...
	require.NoError(t, err)
	wantBody = fmt.Sprintf(`{
		"diamnet_address": "approved-diamnet-address",
...
func TestAPI_deleteKYCStatus(t *testing.T) {
	...
	m := chi.NewMux()
	m.Delete("/kyc-status/{diamnet_address}", handler.ServeHTTP)
