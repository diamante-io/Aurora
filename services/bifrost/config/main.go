...
type Config struct {
	...
	HcNet struct {
		...
		IssuerPublicKey string `valid:"required,hcnet_accountid" toml:"issuer_public_key"`
		...
		DistributionPublicKey string `valid:"required,hcnet_accountid" toml:"distribution_public_key"`
		// SignerSecretKey is:
		...
		// https://www.hcnet.org/developers/guides/channels.html
		// Signer's sequence number will be consumed in transaction's sequence number.
		SignerSecretKey string `valid:"required,hcnet_seed" toml:"signer_secret_key"`
		...
		StartingBalance string `valid:"optional,hcnet_amount" toml:"starting_balance"`
