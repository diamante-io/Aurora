...
func (ac *AccountConfigurator) Start() error {
	ac.log = common.CreateLogger("DiamNetAccountConfigurator")
	ac.log.Info("DiamNetAccountConfigurator starting")
...
func (ac *AccountConfigurator) ConfigureAccount(destination, assetCode, amount string) {
	...
	localLog.Info("Configuring DiamNet account")
	...
	for {
		...
		localLog.WithField("destination", destination).Info("Creating DiamNet account")
		err = ac.createAccountTransaction(destination)
		if err != nil {
			localLog.WithField("err", err).Error("Error creating DiamNet account")
			time.Sleep(2 * time.Second)
			continue
		}
