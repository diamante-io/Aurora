...
const (
	...
	diamnetCorePostgresPassword = "mysecretpassword"
	adminPort                   = 6060
	diamnetCorePort             = 11626
	diamnetCorePostgresPort     = 5641
...
func NewTest(t *testing.T, config Config) *Test {
	...
	// Only run Diamnet Core container and its dependencies.
	i.runComposeCommand("up", "--detach", "--quiet-pull", "--no-color", "core")
	i.prepareShutdownHandlers()
	i.coreClient = &diamnetcore.Client{URL: "http://localhost:" + strconv.Itoa(diamnetCorePort)}
	i.waitForCore()
...
func (i *Test) configureCaptiveCore() {
	...
	if value := i.getParameter(
		aurora.DiamnetCoreBinaryPathName,
...
func (i *Test) StartAurora() error {
	...
	cmd := &cobra.Command{
		...
		Long:  "Client-facing API server for the Diamnet network.",
	...
	defaultArgs := map[string]string{
		"diamnet-core-url": i.coreClient.URL,
		"diamnet-core-db-url": fmt.Sprintf(
			"postgres://postgres:%s@%s:%d/diamnet?sslmode=disable",
			diamnetCorePostgresPassword,
			hostname,
			diamnetCorePostgresPort,
		),
		"diamnet-core-binary-path":      coreBinaryPath,
...
func findDockerComposePath() string {
	...
	if gopath := os.Getenv("GOPATH"); gopath != "" {
		monorepo := filepath.Join(gopath, "src", "github.com", "diamnet", "go")
