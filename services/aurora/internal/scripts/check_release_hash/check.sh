...
apt-get clean
apt-get update
apt-get install -y diamnet-aurora=$PACKAGE_VERSION

mkdir released
cd released

wget https://github.com/diamnet/go/releases/download/$TAG/$TAG-darwin-amd64.tar.gz
wget https://github.com/diamnet/go/releases/download/$TAG/$TAG-linux-amd64.tar.gz
wget https://github.com/diamnet/go/releases/download/$TAG/$TAG-linux-arm.tar.gz
wget https://github.com/diamnet/go/releases/download/$TAG/$TAG-windows-amd64.zip
...
./dist/$TAG-linux-amd64/aurora version

echo "github releases version"
./released/$TAG-linux-amd64/aurora version

echo "debian package version"
diamnet-aurora version
...
do
    ...
    if [ -f "./released/$TAG-$S.tar.gz" ]; then
        released=($(shasum -a 256 ./released/$TAG-$S/aurora))
    else
        # windows
        released=($(shasum -a 256 ./released/$TAG-$S/aurora.exe))
    fi

    if [ -f "./dist/$TAG-$S.tar.gz" ]; then
        dist=($(shasum -a 256 ./dist/$TAG-$S/aurora))
    else
        # windows
        dist=($(shasum -a 256 ./dist/$TAG-$S/aurora.exe))
    fi

    if [ $S == "linux-amd64" ]; then
        path=$(which diamnet-aurora)
