...
const (
	...
	// DiamnetCoreURLFlagName is the command line flag for configuring the URL fore Diamnet Core HTTP endpoint
	DiamnetCoreURLFlagName = "diamnet-core-url"
	// DiamnetCoreBinaryPathName is the command line flag for configuring the path to the diamnet core binary
	DiamnetCoreBinaryPathName = "diamnet-core-binary-path"
	...
	captiveCoreMigrationHint = "If you are migrating from Aurora 1.x.y, start with the Migration Guide here: https://developers.diamnet.org/docs/run-api-server/migrating/"
)
...
func applyMigrations(config Config) error {
	...
	if err != nil {
		return fmt.Errorf("could not connect to aurora db: %v", err)
	}
	defer dbConn.Close()
...
A database migration is required to run this version (%v) of Aurora. Run "aurora db migrate up" to update your DB. Consult the Changelog (https://github.com/diamnet/go/blob/master/services/aurora/CHANGELOG.md) for more information.`,
	...
	if nMigrationsDown > 0 {
		return fmt.Errorf(
			`A database migration DOWN to an earlier version of the schema is required to run this version (%v) of Aurora. Consult the Changelog (https://github.com/diamnet/go/blob/master/services/aurora/CHANGELOG.md) for more information.
...
func Flags() (*Config, support.ConfigOptions) {
	...
	var flags = support.ConfigOptions{
		...
		&support.ConfigOption{
			...
			OptType:   types.String,
			Required:  false,
			Usage:     "aurora postgres read-replica to connect with, when set it will return stale history error when replica is behind primary",
		},
		&support.ConfigOption{
			Name:        DiamnetCoreBinaryPathName,
			OptType:     types.String,
			FlagDefault: "",
			Required:    false,
			Usage:       "path to diamnet core binary (--remote-captive-core-url has higher precedence). If captive core is enabled, look for the diamnet-core binary in $PATH by default.",
		...
		&support.ConfigOption{
			...
			FlagDefault: true,
			Required:    false,
			Usage:       "causes Aurora to ingest from a Captive Diamnet Core process instead of a persistent Diamnet Core database",
		...
		&support.ConfigOption{
			...
			CustomSetValue: support.SetOptionalUint,
			Required:       false,
			Usage:          "port for Captive Core to bind to for connecting to the Diamnet swarm (0 uses Diamnet Core's default)",
		...
		&support.ConfigOption{
			...
			OptType:   types.String,
			Usage:     "diamnet-core to connect with (for http commands). If unset and the local Captive core is enabled, it will use http://localhost:<diamnet_captive_core_http_port>",
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{
			...
			Required:       false,
			Usage:          "name of the path for Core logs (leave empty to log w/ Aurora only)",
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{
			...
			FlagDefault: false,
			Required:    false,
			Usage:       "determines if Aurora instance is behind Cloudflare, in such case client IP in the logs will be replaced with Cloudflare header (cannot be used with --behind-aws-load-balancer)",
		},
		&support.ConfigOption{
			...
			FlagDefault: false,
			Required:    false,
			Usage:       "determines if Aurora instance is behind AWS load balances like ELB or ALB, in such case client IP in the logs will be replaced with the last IP in X-Forwarded-For header (cannot be used with --behind-cloudflare)",
		},
	}
...
func NewAppFromFlags(config *Config, flags support.ConfigOptions) (*App, error) {
	...
	if config.DiamnetCoreURL == "" {
		return nil, fmt.Errorf("flag --%s cannot be empty", DiamnetCoreURLFlagName)
	}
	if config.Ingest && !config.EnableCaptiveCoreIngestion && config.DiamnetCoreDatabaseURL == "" {
		return nil, fmt.Errorf("flag --%s cannot be empty", DiamnetCoreDBURLFlagName)
	}
...
func ApplyFlags(config *Config, flags support.ConfigOptions, options ApplyOptions) error {
	...
	if config.Ingest {
		...
		if config.EnableCaptiveCoreIngestion {
			binaryPath := viper.GetString(DiamnetCoreBinaryPathName)

			// If the user didn't specify a Diamnet Core binary, we can check the
			// $PATH and possibly fill it in for them.
			if binaryPath == "" {
				if result, err := exec.LookPath("diamnet-core"); err == nil {
					binaryPath = result
					viper.Set(DiamnetCoreBinaryPathName, binaryPath)
			...
			if binaryPath == "" && config.RemoteCaptiveCoreURL == "" {
				return fmt.Errorf("Invalid config: captive core requires that either --%s or --remote-captive-core-url is set. %s",
					DiamnetCoreBinaryPathName, captiveCoreMigrationHint)
			}
			...
			if config.RemoteCaptiveCoreURL == "" && (binaryPath == "" || config.CaptiveCoreConfigPath == "") {
				if options.RequireCaptiveCoreConfig {
					...
					errorMessage := fmt.Errorf(
						...
						DiamnetCoreBinaryPathName, CaptiveCoreConfigPathName, captiveCoreMigrationHint,
					)
					...
					case network.TestNetworkPassphrase:
						...
						config.HistoryArchiveURLs = []string{"https://history.diamnet.org/prd/core-testnet/core_testnet_001/"}
					case network.PublicNetworkPassphrase:
						configFileName = "captive-core-pubnet.cfg"
						config.HistoryArchiveURLs = []string{"https://history.diamnet.org/prd/core-live/core_live_001/"}
			...
			if config.DiamnetCoreURL == "" && config.RemoteCaptiveCoreURL == "" && config.CaptiveCoreToml.HTTPPort != 0 {
				config.DiamnetCoreURL = fmt.Sprintf("http://localhost:%d", config.CaptiveCoreToml.HTTPPort)
				viper.Set(DiamnetCoreURLFlagName, config.DiamnetCoreURL)
			}
		}
	} else {
		if config.EnableCaptiveCoreIngestion && (config.CaptiveCoreBinaryPath != "" || config.CaptiveCoreConfigPath != "") {
			...
			return fmt.Errorf("Invalid config: one or more captive core params passed (--%s or --%s) but --ingest not set. "+captiveCoreMigrationHint,
				DiamnetCoreBinaryPathName, captiveCoreConfigFlag)
		}
		if config.DiamnetCoreDatabaseURL != "" {
			return fmt.Errorf("Invalid config: --%s passed but --ingest not set. ", DiamnetCoreDBURLFlagName)
		}
	}
	...
	if config.BehindCloudflare && config.BehindAWSLoadBalancer {
		return fmt.Errorf("Invalid config: Only one option of --behind-cloudflare and --behind-aws-load-balancer is allowed. If Aurora is behind both, use --behind-cloudflare only.")
	}
