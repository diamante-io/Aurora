...
var _ diamnetCoreClient = (*mockDiamnetCore)(nil)

type mockDiamnetCore struct {
	mock.Mock
}

func (m *mockDiamnetCore) Info(ctx context.Context) (*diamnetcore.InfoResponse, error) {
	args := m.Called(ctx)
	return args.Get(0).(*diamnetcore.InfoResponse), args.Error(1)
}

func TestHealthCheck(t *testing.T) {
	synced := &diamnetcore.InfoResponse{}
	synced.Info.State = "Synced!"
	notSynced := &diamnetcore.InfoResponse{}
	...
	for _, tc := range []struct {
		...
		pingErr          error
		coreErr          error
		coreResponse     *diamnetcore.InfoResponse
		expectedStatus   int
		...
		},
		{
			"diamnet core down",
			nil,
			fmt.Errorf("diamnet core is down"),
			nil,
			http.StatusServiceUnavailable,
			healthResponse{
				DatabaseConnected: true,
		...
		{
			"diamnet core not synced and db down",
			fmt.Errorf("database is down"),
			nil,
			notSynced,
			http.StatusServiceUnavailable,
...
func TestHealthCheckCache(t *testing.T) {
	...
	core := &mockDiamnetCore{}
	core.On("Info", h.ctx).Return(&diamnetcore.InfoResponse{}, fmt.Errorf("core err")).Once()
	h.session = session
	h.core = core
