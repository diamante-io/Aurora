...
func checkPaths(paths []string) {
	for _, path := range paths {
		a := cmp.NewResponse(auroraBase, path, false)
		b := cmp.NewResponse(auroraTest, path, false)
...
func getAllPagesPaths(page string) []string {
	...
	for {
		resp, err := http.Get(auroraBase + page)
		if err != nil {
			panic(err)
		}
		...
		page = strings.Replace(page, auroraBase, "", -1)
		...
		// no extra objects in the Aurora we are testing.
