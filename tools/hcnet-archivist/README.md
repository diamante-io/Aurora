...
This is a small tool, written in Go, for working with `hcnet-core` history archives directly.
It is a standalone tool that does not require `hcnet-core`, or any other programs.

It is much smaller and simpler than `hcnet-core`, and is intended only for archive-maintenance tasks.
...
```
inspect hcnet history archive

Usage:
  hcnet-archivist [flags]
  hcnet-archivist [command]
...
Flags:
  ...
  -h, --help              help for hcnet-archivist
...
Use "hcnet-archivist [command] --help" for more information about a command.
```

## Specifying history archives

Unlike `hcnet-core`, `hcnet-archivist` does not run subprocesses to access history archives;
...
`hcnet-archivist` supports reading from and writing to any S3-compatible storage.
...
```
$ hcnet-archivist status --s3endpoint ams3.digitaloceanspaces.com s3://bucketname/prefix
```
...
$ hcnet-archivist status --s3endpoint https://storage.googleapis.com s3://google-storage-bucketname
``` 
...
```
$ hcnet-archivist status http://s3-eu-west-1.amazonaws.com/history.hcnet.org/prd/core-testnet/core_testnet_001

       Archive: http://s3-eu-west-1.amazonaws.com/history.hcnet.org/prd/core-testnet/core_testnet_001
...
```
$ hcnet-archivist mirror http://s3-eu-west-1.amazonaws.com/history.hcnet.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 18:27:09 mirroring http://s3-eu-west-1.amazonaws.com/history.hcnet.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ hcnet-archivist --last 1024 mirror http://s3-eu-west-1.amazonaws.com/history.hcnet.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 19:14:01 mirroring http://s3-eu-west-1.amazonaws.com/history.hcnet.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ hcnet-archivist scan file://local-archive
...
```
$ hcnet-archivist --last 4096 scan file://local-archive
...
$ hcnet-archivist --low 1000000 --high 1006000 scan file://local-archive
...
```
$ hcnet-archivist --verify --last 4096 scan file://local-archive
...
$ hcnet-archivist repair file://local-archive file://broken-archive
...
```
 hcnet-archivist dumpxdr local-archive/transactions//00/20/de/transactions-0020de7f.xdr.gz
